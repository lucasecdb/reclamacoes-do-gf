...
## Definição da eficácia dos léxicos

```{r}
reclamacoes_l %>%
    group_by(lexico) %>%
    summarise(rmse = sqrt(mean(erro)))
```
...
ggpairs(reclamacoes_l %>% select(lexico, orgao, orgao.dummy, erro))

lm1 = lm(erro ~ orgao.dummy + lexico.dummy, data = reclamacoes_l)

plot(reclamacoes_l$orgao.dummy, reclamacoes_l$erro)
abline(lm1)
summary(lm1)
